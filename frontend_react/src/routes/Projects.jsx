...
const Projects = () => {
  ...
  useEffect(() => {
    const fetchProjects = async () => {
      try {
        const response = await axios.post('https://dcm-backend.vercel.app/student_projects/', { cookie: cookie });
        ...
        <tbody>
          {projects.map((project, index) => (
            <tr>
                ...
                <td style={{ border: '1px solid #ddd', padding: '12px' }}>{project.semester}</td>
